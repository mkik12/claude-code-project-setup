...
# A trailing "'s" is a contraction only after these words. Anywhere else it is
# a possessive ("the user's language"), which STE permits.
CONTRACTED_S = (r"\b(?:it|he|she|that|there|here|what|who|where|when|how|why"
                r"|this|let|one|somebody|someone|everybody|everyone|nobody"
                r"|nothing|something)['’]s\b")
...
def lint(text, strict=False):
    ...
    v["contraction"] = len(re.findall(r"\b\w+['’](?:t|re|ve|ll|d|m)\b", text)) \
        + len(re.findall(CONTRACTED_S, text, re.I))
