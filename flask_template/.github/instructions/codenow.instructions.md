---
description: 'The CodeNow platform contract, logging rules, and read-only-app-directory rule. Every project publishes to CodeNow.'
applyTo: '**'
---

# CodeNow

Read this before any work; every project built with this framework publishes to
CodeNow. It is the single home for CodeNow guidance: what a component gives you,
the contract you must not break, and the operational rules.
...
following the Flask template rule (`flask.instructions.md`).
...
when the app needs it, and record it in `requirements.txt` pinned to a specific
version (matching the component's pinned style).
...
user publishes manually in CodeNow.
