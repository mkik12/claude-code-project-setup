...
## Environment: check before you choose a route

This skill has two routes to a new deck, and its Visual QA needs a renderer.
Check what the machine has first:
...
| Condition | Route |
| :--- | :--- |
| `node` is present | pptxgenjs, described below. |
| `node` is absent | python-pptx. See [references/python-pptx.md](references/python-pptx.md). |

python-pptx has one advantage worth knowing even when Node is available: it
writes chart XML that PowerPoint opens. pptxgenjs writes chart XML that
PowerPoint refuses and every other tool accepts, which is the trap described
under QA below.
...
| Condition | Route |
| :--- | :--- |
| `soffice` is present | "Converting to Images", then all of Visual QA. |
| `soffice` is absent | `python scripts/office/validate.py output.pptx` plus the Content QA checks. Nothing renders the deck, so never report it as visually checked. |

Without a renderer, text overflow is both the most common defect and the one you
cannot see. Leave 10% slack in every text box and say the layout is unverified.
A legacy `.ppt` also needs `soffice`, so ask the user to save it as `.pptx`.

On the machine this was built for, `node` and `soffice` are both absent, so
python-pptx and the XSD check are the defaults. Check rather than assume.
...
**No `zip` on this machine?** It is missing far more often than `unzip` is.
`python scripts/repack.py unpacked/ out.pptx` does the same job with the
standard library: it writes a fresh archive, so a part you deleted does not
survive, and it puts `[Content_Types].xml` first. Delete a part and you must
also delete the relationship that points at it, or the package is broken and
`validate.py` says so. `clean.py` handles that for a deleted slide.
