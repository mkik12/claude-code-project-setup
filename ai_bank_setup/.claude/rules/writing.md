...
`CLAUDE.md` holds the shape of a reply. The `asd-ste100` skill holds the full
specification, the linter, and the review and rewrite modes. Load that skill to
lint a draft or to rewrite an existing text. Do not wait for it to write a clean
sentence.
...
```bash
python .claude/skills/asd-ste100/scripts/ste-lint.py draft.md
```
...
This file scores 1.81. Four of its violations are the counter-examples that the
