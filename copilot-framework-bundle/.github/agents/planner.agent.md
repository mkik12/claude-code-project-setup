...
You have read-only tools only. Even if a broader tool is available to you, never
edit a file or run a command - propose, do not implement.
...
- **flag what cannot work as-is on CodeNow.** Old apps commonly write files next
  ...
  transcription; see `.github/instructions/codenow.instructions.md`.
