...
## Environment: check whether formulas can be computed

Everything here uses openpyxl, so there is one route to a spreadsheet. What
changes with the machine is whether a formula can be **computed**, because
`scripts/recalc.py` drives LibreOffice. Check first:
...
| :--- | :--- |
| `soffice` is present | "Recalculate (mandatory whenever the file contains formulas)" applies in full. Read the rest of this file as written. |
| `soffice` is absent | `recalc.py` cannot run. Read the three points below, which override anything conflicting further down. |

### Without a recalculation step

**1. A formula you write carries no cached value.** `pandas`,
`load_workbook(data_only=True)`, and most previewers read that cell as `None`
until Excel opens the file and computes it. Choose by who reads the file next:

- **a person, in Excel.** Write the formulas. Excel computes them on open. Say
  that Excel computes the values, and that you did not.
- **code, or a preview.** Compute the values in Python and write numbers. Add a
  formula only where the user asked for a live model.

**2. You cannot verify that a formula evaluates.** Keep the formulas simple.
Write two or three first and check the ranges by hand. Say that they are
unverified.

**3. Read "Choosing formulas that survive verification" as follows.** Part of it
still holds and part of it is void:

- **The `_xlfn.` prefix rule holds.** It is not a LibreOffice rule. Excel itself
  stores a post-2007 function name with the prefix, and openpyxl writes your
  string into the XML verbatim, so a bare `TEXTJOIN` gives `#NAME?` in Excel.
- **The ban on `XLOOKUP`, `XMATCH`, `SORT`, `FILTER`, `UNIQUE` and `SEQUENCE` is
  lifted.** It existed because LibreOffice could not evaluate them. Excel
  evaluates all six. They are post-2007 functions, so the prefix rule above
  applies to them: `_xlfn.XLOOKUP`, and a spilling function such as `SORT` takes
  a different prefix again. Confirm the exact prefixed name before you write one.
- **The lowercased-formula tell is void.** LibreOffice wrote it.

On the machine this was built for, `soffice` is absent. Check rather than
assume.
