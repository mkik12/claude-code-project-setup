...
Engineering principles and formatting for the code the agents write. The planner
and coder read this file before writing code.
...
- Plain CSS in `static/` - no preprocessors or frameworks.
...
- Plain, modern JavaScript in `static/` - no frameworks and no build step.
- `const`/`let`, never `var`; `===` over `==`; `camelCase` names.
- Put page scripts at the end of `<body>` or in a module under `static/`.
- Keep DOM logic small; if a page needs heavy interactivity, reconsider the
  template choice with the planner.
